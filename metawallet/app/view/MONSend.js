...
Ext.define('FW.view.MONSend', {
    ...
    getTokenInfo: function(asset){
        ...
        if(asset=='MON'){
            var price_usd = me.main.getCurrencyPrice('monero','usd'),
                price_btc = me.main.getCurrencyPrice('counterparty','mon');
            me.tokenInfo = {
                asset: 'MON',
                estimated_value : {
                    MON: 1.00000000,
                    usd: price_usd,
                    xcp: (price_mon) ? numeral(1 / price_mon).format('0.00000000') : '0.00000000'
                }
            };
            me.price.enable();
        } else {
    ...
    updateBalance: function(asset){
        var me      = this,
            store   = Ext.getStore('MONBalances'),
            prefix  = FW.MONWALLET_ADDRESS.address.substr(0,5);
            balance = 0,
            values  = false,
            format  = '0,0';
